...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* Mobile menu overlay */}
      <div
        className={`md:hidden fixed inset-0 bg-foreground/20 backdrop-blur-sm z-[60] transition-opacity duration-300 ${mobileOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
        onClick={() => setMobileOpen(false)}
      />

      {/* Mobile menu panel */}
      <div
        className={`md:hidden fixed top-0 right-0 h-full w-[75%] max-w-xs bg-[hsl(0,0%,100%)] z-[70] shadow-xl transition-transform duration-300 ease-out ${mobileOpen ? "translate-x-0" : "translate-x-full"}`}
      >
